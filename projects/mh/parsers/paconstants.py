...
RE_ATOM_COUNT = r'\s*<tr id=.ch.*A.*C.*t.>\s*<th>A.*C.*t</th>\s*<td>(.*)</td>\s*</tr>\s*<tr ' \
                r'id=.ch.*Ch.*A.*C.*.>\s*<th>'
RE_BOND = r'<tr.id=.c[a-z]+B.+C.+t.>\s*<th>B.+\sC.+t</th>\s*<td>(\d*)</td>\s*</tr>\s*<tr.' \
          r'id=.ch.+A.+A.+C.+t.>'
RE_CHARGE = r'<tr id=.ch.+Fo.+C.+e.>\s*<th>Fo.+\sC.*e</th>\s*<td>([\w\.\-\+]+)</td>'
RE_CHEBI = r">CHEBI:([0-9]*)"
RE_CHEMBL = r">CHEMBL([0-9]*)"
RE_CHIRAL = r'<tr.id=.c[a-z]+C.+A.+t.>\s*<th>C[a-z]+\sAt.+Co.+t</th>\s*<td>(\d+)</td>\s*</tr>\s*' \
            r'<tr.id=.c.+B\w+C\w+t.>'
RE_COMP_ID = r'(NPC\S*)'
...
RE_KNAP_ENTRY = r'<tr>[.|\s]*<td\sc.*d1\">[.|\s]*<a.*blank\">(C[0-9]*)</a>[.|\s]*</td>[.|\s]*' \
                r'<td.*d1\">([ 0-9|-]*)</td>\s*<td\s.*d1\">(.*)</td>[.|\s]*<td\s.*d1\">' \
                r'([0-9|A-Z]+)</td>[.|\s]*<td\s.*d1\">([\d|\.]+).*(<[|/]td.*<[|/]tr>) '
RE_KNAP_NAME = r'<th class=\"inf\">Name</th>\s*<td colspan=\"4\" class=\"inf\">(.*)</td>\s*</tr>'
RE_KNAP_ORG = r'.*</td><td class=\"?org2\"?>([a-zA-Z0-9!@#$&()\\-`.+,/\"\s]*)'
RE_NAME_LONG = r'id=.ch[a-z]+I[a-z]+s.>\s*<th>Id.+rs</th>\s*<td.s.+=.word.+word.>(.*)</td></tr>' \
...
RE_SMILES = r'id=.ch.*Is.*c.>\s*<th>Is.*S.*S</th>\s*<td.*\">(.*)</td>\s*</tr>\s*' \
            r'<tr.id=.ch.*I.*I.>\s*'
RE_WEIGHT = r'.*<tr id=.ch.*Mo.*W.*t.>\s*<th>Mo.*W.*t</th>\s*<td>(.*)</td>\s*</tr>\s*' \
            r'<tr id=\"ch.*lT.*e\">'

# knapsack partial URL
URL_KNAP_ORG = "http://www.knapsackfamily.com/knapsack_core/result.php?sname=organism&word="
...
PROP_PAIRS = [('IUPACName', 'IUPAC'), ('InChI', 'InChI'), ('InChIKey', 'InChIKey'),
              ('CanonicalSMILES', 'CannonSMILES'),
              ('IsomericSMILES', 'IsoSMILES'), ('MolecularFormula', 'Formula'),
              ('MolecularWeight', 'Weight'),
              ('Charge', 'Charge'), ('Fingerprint2D', 'Fingerprint2D'),
              ('RotatableBondCount', '#RotatableBonds'),
              ('HeavyAtomCount', '#HeavyAtoms'), ('IsotopeAtomCount', '#IsotopeAtoms'),
              ('AtomStereoCount', '#StereoAtoms'), ('DefinedAtomStereoCount', '#DefStereoAtoms'),
              ('UndefinedAtomStereoCount', '#UndefStereoAtoms'),
              ('BondStereoCount', '#StereoBonds'),
              ('DefinedBondStereoCount', '#DefStereoBonds'),
              ('UndefinedBondStereoCount', '#UndefStereoBonds'),
              ('CovalentUnitCount', '#CovalentUnits'), ('HBondDonorCount', '#HBondDonors'),
              ('HBondAcceptorCount', '#HBondAcceptors')]
XREF_PAIRS = [('RN', 'CAS ID(S)'), ('MMDBID', 'MMDBID(S)'), ('ProteinGI', 'ProtGI(S)'),
              ('NucleotideGI', 'NTGI(S)'),
              ('TaxonomyID', 'TaxID(S)'), ('MIMID', 'MIMID(S)'), ('GeneID', 'GeneID(S)'),
              ('ProbeID', 'ProbeID(S)')]
MISC_PAIRS = [('ZINC', 'ZINC ID'), ('CTK', 'ChemTik ID'), ('CHEMBL', 'CHEMBL ID'),
              ('CHEBI:', 'CHEBI ID'),
