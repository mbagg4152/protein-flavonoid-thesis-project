...
def get_json_data(file_name, key=None):
   """
   This function uses the python JSON library in order to parse JSON files into usable
   python objects. Can return lists or dictionaries, depending on the JSON file's
   structure.
   """
...
def unique_element_list(list_name, index):
   ...
   for i in list_name:
      # finds unique EC num not in the list & adds it to the list
      if original_index == 'last': index = len(i) - 1
...
def list_partition(seq, num):
   """
   This function takes in a list and then splits it into as many parts as specified
   by parameter num
   """
...
def write_readme(main_dir, readme, init_time, fasta_path, gene_path):
   ...
   with open(main_dir + readme, 'w') as readme_doc:
      ...
      readme_doc.write('This script creates a series of files related to the genes ' +
                       'associated with plant flavonoids from various species of ' +
                       'plants. This script first creates the MasterCount and ' +
                       'MasterList files;  the MasterCount counts the number genes ' +
                       'each plant species have that correspond with each EC number; ' +
                       'while the MasterList lists every gene with number for each ' +
                       'plant specie. These are located in ' + os.getcwd() + '. The ' +
                       'script also creates files that only contains the genes of a ' +
                       'single plant species biochemical pathway which are located in ' +
                       gene_path + '. The script also creates a Master FASTA files ' +
                       'which contains the DNA sequence of each gene and FASTA files ' +
                       'organized by EC number, these are located in ' + fasta_path)
      readme_doc.close()
...
def write_append(path, content, write_over=None, skip=None):
   """
   This function takes a file name and the contents to be written to a file. If the
   file doesn't exist, it is created then written to. If it does exist, then it is
   appended to. The optional arg write_over is used for when the file exists and the
   content needs to be written over.
   """
   try:
      file = open(path, 'x')
      file.close()
