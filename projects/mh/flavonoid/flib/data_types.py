# custom project library import
from flib.fconstants import *


class Flav:
   """
   This class holds the data for each flavonoid. The objects are initialized with their
   file name and label and only later in the program, their empty list of plants will
   be filled.
   -------------------------------------------------------------------------------------
   ATTRIBUTES
...
class Plant:
   """
   This object holds information about each plant used in the program. The plant objects
   are initialized with their scientific name and their code and then have different
   ...
   __eq__: defines equality of the object
   is_in: determines if an identical or nearly identical object is already in the list
   has_ec_count: used to determine whether or not a specific EC number is already in the
                 list of EC counts
   ...
   def simple(self):
      ...
      return self.name + ', ' + self.code + ', ' + gstr + ', ' + \
             str(self.ec_nums) + ', ' + str(self.flavonoids)
...
class PathGene:
   """
   This object is used to hold Gene objects in a way such that they are sorted by the
   pathway from which they were found.
   --------------------------------------------------------------------------------------
   ATTRIBUTES
...
class Gene:
   ...
   def __init__(self, gene_id=None, plant=None, compound=None, ec_nums=None,
                ortho=None, path=None, plant_code=None):
   ...
   def simple(self):
      return self.plant + ', ' + self.gene_id + ', ' + self.compound + ', ' + \
             str(self.ec_nums) + ', ' + self.ortho
...
class EcCounts:
   """
   This object is a property of the Plant object and is used to hold each EC number and
   the number of times it occurs in gene entries of a given plant.
   --------------------------------------------------------------------------------------
   ATTRIBUTES
...
class FastaEcEntry:
   """
   This object is a property of EcFastaCollection and contains the information for a
   specific FASTA entry.
   --------------------------------------------------------------------------------------
   ATTRIBUTES
